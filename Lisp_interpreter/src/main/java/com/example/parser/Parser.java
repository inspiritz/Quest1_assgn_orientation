...
public class Parser {
    ...
    public Parser(List<Token> tokens, NodeFactory nodeFactory) {
        this.tokens = tokens;
        this.current = 0;
    ...
    public Node parseExpression() {
        ...
        if (token.getType().equals("NUMBER")) {
            current++;
            return nodeFactory.createNumber(Integer.parseInt(token.getToken()));
        }

        if (token.getType().equals("SYMBOL")) {
            current++;
            return nodeFactory.createSymbol(token.getToken());
        }
        ...
        if (token.getType().equals("LPAREN")) {
            ...
            current++;
            return nodeFactory.createList(operator, children);
        }
